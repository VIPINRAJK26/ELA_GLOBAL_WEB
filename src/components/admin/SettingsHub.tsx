...
export default function SettingsHub() {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error && !error.isNetworkError) {
        setPwdFeedback('Error updating password: ' + error.message);
        return;
      }
    } catch (err: any) {
      console.warn("Password change offline fallback in SettingsHub:", err);
    }

    // Perform update in local session and staff registry
    const rStr = localStorage.getItem('ilas_staff_registry');
    ...
    setPwdFeedback('Profile and password updated successfully.');
    setOldPassword('');
    setNewPassword('');
  };
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    // Create user in DRF if available
    try {
      const { error } = await staffService.createStaff({
        email: newStaffEmail.trim(),
        name: newStaffName.trim(),
        department: newStaffDept as any,
        hrIssuedId: newStaffHRId.trim(),
        password: newStaffPassword,
      });
      if (error && !error.isNetworkError) {
        setStaffFeedback('DRF Staff Creation Error: ' + error.message);
        return;
      }
    } catch (err: any) {
      console.warn("DRF staff creation offline fallback in SettingsHub:", err);
    }
